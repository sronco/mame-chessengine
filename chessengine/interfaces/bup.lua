...
function interface.setlevel()
	...
	if     (n == 1) then send_input(":KEY.0", 0x01, 1)
	elseif (n == 2) then send_input(":KEY.0", 0x02, 1)
	elseif (n == 3) then send_input(":KEY.0", 0x04, 1)
	elseif (n == 4) then send_input(":KEY.0", 0x08, 1)
	elseif (n == 5) then send_input(":KEY.0", 0x10, 1)
	elseif (n == 6) then send_input(":KEY.0", 0x20, 1)
	end
	send_input(":KEY.1", 0x02, 1) -- ENT
end

function interface.setup_machine()
	sb_reset_board(":board:board")
	emu.wait(1.0)
	while (output:get_indexed_value("led", 101) == 0) do
		machine:soft_reset()
		emu.wait(1.0)
	end
	send_input(":KEY.1", 0x01, 1) -- CL
	emu.wait(1.0)
...
function interface.start_play(init)
	if (output:get_indexed_value("led", 100) ~= 0) then
		send_input(":KEY.0", 0x80, 1) -- BLACK
	elseif (output:get_indexed_value("led", 101) ~= 0) then
		send_input(":KEY.0", 0x40, 1) -- WHITE
	end
...
function interface.promote(x, y, piece)
	...
	if     (piece == "q" or piece == "Q") then	send_input(":KEY.0", 0x02, 1)
	elseif (piece == "r" or piece == "R") then	send_input(":KEY.0", 0x04, 1)
	elseif (piece == "b" or piece == "B") then	send_input(":KEY.0", 0x08, 1)
	elseif (piece == "n" or piece == "N") then	send_input(":KEY.0", 0x10, 1)
	end
end
