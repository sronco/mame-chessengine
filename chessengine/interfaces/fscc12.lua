...
function interface.setlevel()
	...
	for y=0,7 do
		if output:get_indexed_value("1.", 7-y) ~= 0 then
...
function interface.clear_announcements()
	local lastval = {}
	for i=1,6 do
		lastval[i] = output:get_indexed_value("0.", i)
	end

	emu.wait(0.25)

	for i=1,6 do
		if (output:get_indexed_value("0.", i) ~= lastval[i]) then
...
function interface.is_selected(x, y)
	...
	return output:get_indexed_value("1.", 8 - y) ~= 0 and interface.is_led_on("0.", x - 1)
end
